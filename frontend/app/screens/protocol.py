...
from app.components.body_map import BodyMap
from app.components.card import AppCard, section_title
from app.components.chips import Chip
from app.theme import ACCENT, SUCCESS, TEXT_SECONDARY, TEXT_TERTIARY, WARNING
from app.utils.async_loader import load_screen
from app.utils.constants import STOCK_WARNING_THRESHOLD
from app.utils.feedback import show_snack


def protocol_screen(page: ft.Page, api: ApiClient, navigate: Callable[[int], None]) -> ft.Control:
    ...
    def draw() -> None:
        def build() -> list[ft.Control]:
            ...
            try:
                ...
                schedule = {item["id"]: api.get(f"/protocol/items/{item['id']}/schedule", {"date": today.isoformat()}) for item in items}
            ...
            for title, group in grouped.items():
                ...
                view.controls.append(AppCard(ft.Column(rows, spacing=10, tight=True)))
            ...
            def select_zone(zone: str) -> None:
                ...
                def add_injection(zone: str) -> None:
                    ...
                    if injection is None:
                        show_snack(page, "Нет инъекционных препаратов", error=True)
                        return
                    page.pop_dialog()
            ...
            view.controls.append(AppCard(ft.Column([
                section_title("Инъекции", ft.FilledButton("Авто-ротация", icon=ft.Icons.AUTORENEW, on_click=lambda _: show_snack(page, "Предложенная зона: " + rotation["suggestion"]))),
    ...
    def log_form(item: dict, preset_site: str | None = None, preset_side: str | None = None, after_save: Optional[Callable[[], None]] = None) -> None:
        ...
        def submit(force: bool = False) -> None:
            try:
                ...
                page.pop_dialog(); show_snack(page, "Приём сохранён")
                if after_save:
                    after_save()
                else:
                    draw()
                ...
                else: show_snack(page, str(exc), True)
    ...
    def detail(item: dict) -> None:
        ...
        dialog = ft.AlertDialog(title=ft.Text(item["name"]), content=ft.Column([ft.Text(f"План: {item['planned_dose_value']} {item['planned_dose_unit']}"), ft.Text(f"Остаток: {stock if stock is not None else 'не ведётся'}"), ft.ProgressBar(value=progress, color=WARNING if stock is not None and stock <= STOCK_WARNING_THRESHOLD else ACCENT), ft.Text("История", weight=ft.FontWeight.W_600), *[ft.Text(f"{entry['date']} {entry['time'][:5]} · {entry['actual_dose_value']} {entry['actual_dose_unit']}", size=12) for entry in history[:8]]], tight=True, scroll=ft.ScrollMode.AUTO), actions=[ft.TextButton("Редактировать", on_click=lambda _: (page.pop_dialog(), edit_item(item))), ft.TextButton("Закрыть", on_click=lambda _: page.pop_dialog())])
        page.show_dialog(dialog)
