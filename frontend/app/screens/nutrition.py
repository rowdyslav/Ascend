...
from app.components.progress import ProgressBar
from app.components.sheets import show_bottom_sheet
from app.theme import ACCENT, PURPLE, SUCCESS, TEXT_SECONDARY, WARNING
from app.utils.async_loader import load_screen
from app.utils.constants import goals_for
...
def nutrition_screen(page: ft.Page, api: ApiClient, navigate: Callable[[int], None]) -> ft.Control:
    ...
    def _build_meal_cards(diary: dict) -> list[ft.Control]:
        cards = []
        for meal_type, label in MEALS:
            meal = next((m for m in diary["meals"] if m["meal_type"] == meal_type), None)
            entries = meal.get("entries", []) if meal else []
            rows = []
            ...
            cards.append(AppCard(ft.Column([section_title(label, actions), *(rows or [ft.Text("Пока пусто", color=TEXT_SECONDARY)])], spacing=10, tight=True)))
        return cards

    def draw() -> None:
        def build() -> list[ft.Control]:
            nonlocal recent_foods
            try:
                diary = api.get("/nutrition/daily-diary", {"date": selected_day.isoformat()})
                day_info = api.get(f"/days/{selected_day.isoformat()}")
            except ApiError as exc:
                return [ft.Text(str(exc))]
            ...
            for meal in diary["meals"]:
                for entry in meal.get("entries", []):
                    ...
                    if food.get("id") and food["id"] not in {f["id"] for f in recent_foods}:
            ...
            for label, key, unit in labels:
                fact = diary["totals"].get(key) or 0
            ...
            view.controls.append(AppCard(ft.Column([section_title("Дневные цели"), *progress_rows], spacing=10, tight=True)))
            ...
            view.controls.append(AppCard(ft.Column([
            ...
            view.controls.extend(_build_meal_cards(diary))
            ...
            if recipes:
                ...
                view.controls.append(AppCard(ft.Column([section_title("Мои рецепты"), *recipe_rows], spacing=10, tight=True)))
            ...
            view.controls.append(AppCard(ft.Column([section_title("Графики", periods), charts_view()], spacing=10, tight=True)))

            return [view]

        load_screen(page, build, root)
    ...
    def save_recipe(meal_type: str) -> None:
        ...
        def save(_: ft.ControlEvent) -> None:
            if not name.value:
                show_snack(page, "Введите название", True)
                return
            try:
            ...
            page.pop_dialog()
            show_snack(page, "Рецепт сохранён")
            draw()
    ...
    def apply_recipe(recipe_name: str) -> None:
        ...
        try:
            ...
            show_snack(page, f"Рецепт «{recipe_name}» добавлен")
            draw()
        except ApiError as exc:
            show_snack(page, str(exc), True)
    ...
    def copy_from_date(_: ft.ControlEvent) -> None:
        ...
        def run(_: ft.ControlEvent) -> None:
            try:
                ...
                show_snack(page, "Неверный формат даты", True)
                return
            try:
                ...
                for source_meal in source:
                    ...
                    for entry in api.get(f"/meals/{source_meal['id']}/entries"):
                        ...
                        copied += 1
                page.pop_dialog()
                show_snack(page, f"Скопировано записей: {copied}" if copied else "На эту дату приёмов нет")
                draw()
            except ApiError as exc:
                show_snack(page, str(exc), True)
