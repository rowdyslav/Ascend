...
from app.components.inputs import AppCheckbox
from app.components.progress import ProgressBar
from app.theme import ACCENT, COLORS, SUCCESS, TEXT, TEXT_SECONDARY, TEXT_TERTIARY, WARNING
...
def today_screen(page: ft.Page, api: ApiClient, navigate: Callable[[int], None]) -> ft.Control:
    ...
    def build() -> list[ft.Control]:
        ...
        def show_metric_sheet(label: str, key: str, unit: str) -> None:
            ...
            def save(_: ft.ControlEvent) -> None:
                try:
                    ...
                    page.pop_dialog()
                    show_snack(page, "Метрика сохранена")
                    navigate(0)
        ...
        controls.append(ft.Column([section_title("Метрики"), ft.Row(metric_tiles, scroll=ft.ScrollMode.AUTO)]))
        ...
        controls.append(AppCard(ft.Column([section_title("Питание", ft.TextButton("Открыть", on_click=lambda _: navigate(2))), *bars], spacing=10, tight=True), onTap=lambda _: navigate(2)))
        ...
        controls.append(AppCard(ft.Column([section_title("Протокол", ft.TextButton("Все", on_click=lambda _: navigate(3))), *(protocol_rows or [ft.Text("Нет обязательных приёмов", color=TEXT_SECONDARY)])], spacing=10, tight=True)))

        # Daily checks: local UI state with all required input types.
        check_active = AppCheckbox(False, lambda _value: None, "Активность выполнена")
        ...
        controls.append(AppCard(ft.Column([section_title("Ежедневные проверки"), check_active, steps, wake, comment, stress], spacing=10, tight=True)))

        def close_day(_: ft.ControlEvent) -> None:
            if day.get("closed"):
                show_snack(page, "День уже закрыт", error=True)
                return
        ...
        controls.append(PrimaryButton("Закрыть день" if not day.get("closed") else "День закрыт ✓", icon=ft.Icons.LOCK, on_click=close_day, disabled=bool(day.get("closed"))))
        return controls
