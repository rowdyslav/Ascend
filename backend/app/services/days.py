...
async def get_or_create_day(user_id, day_date: date) -> Day:
    ...
    if day is None:
        day = Day(user_id=user_id, date=day_date, updated_at=datetime.utcnow())
        await day.insert()
    return day
...
async def refresh_completion(user_id, day_date: date) -> Day:
    ...
    active_items = [
        ...
        if item.is_required
        and (item.end_date is None or item.end_date >= day_date)
        and (not item.weekdays or day_date.isoweekday() in item.weekdays)
    ]
    ...
    entry_count = 0
    for meal in meals:
        entry_count += await FoodEntry.find(FoodEntry.meal_id == meal.id).count()
